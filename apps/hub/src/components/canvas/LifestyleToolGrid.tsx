import type { LifestyleToolCategory, LifestyleToolId } from '../../config/lifestyleCanvasTools';
import {
  LIFESTYLE_CANVAS_TOOLS,
  LIFESTYLE_CATEGORY_META,
  lifestyleToolsByCategory,
} from '../../config/lifestyleCanvasTools';

interface Props {
  onSelectTool: (toolId: LifestyleToolId) => void;
}

const CATEGORY_ORDER: LifestyleToolCategory[] = ['body', 'nutrition', 'dates', 'wellness'];

export default function LifestyleToolGrid({ onSelectTool }: Props) {
  const grouped = lifestyleToolsByCategory();
  ...
  return (
    <div className="space-y-8">
      {CATEGORY_ORDER.map((category) => {
        ...
        const meta = LIFESTYLE_CATEGORY_META[category];
              ...
              {tools.map((tool) => (
                <li key={tool.id}>
                  ...
                  >
                    ...
                    <div>
                      <p className="text-base font-semibold text-canvas-text group-hover:text-violet-200">
                        {tool.label}
                      </p>
      ...
      <p className="rounded-xl border border-canvas-border bg-canvas-elevated px-4 py-3 text-center text-xs font-medium leading-relaxed text-slate-300">
        {LIFESTYLE_CANVAS_TOOLS.length} wellness tools · 100% client-side · private on-device storage
      </p>
    </div>
  );
}
