...
interface Props {
  actions: MediaCanvasAction[];
  onActionClick?: (actionId: string) => void;
}

export default function MediaActionToolbar({ actions, onActionClick }: Props) {
  if (actions.length === 0) {
    return (
    ...
    <div className="rounded-2xl border border-canvas-border bg-canvas-surface p-4 shadow-none sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm font-medium text-canvas-text">Choose what to do with this image</p>
        </div>
              ...
              <button
                ...
                className={`group relative flex w-full flex-col items-start gap-2 rounded-xl border px-3 py-3 text-left transition active:scale-[0.98] ${
                  isPro
                    ...
                    : 'border-canvas-border bg-canvas-elevated hover:border-violet-300 hover:bg-canvas-surface hover:shadow-none'
                }`}
              >
                {isPro && (
