...
export default function QuickDecisionWheel() {
  const initial = useMemo(
    () => loadPersistedJson<FormState>(QUICK_TOOLS_STORAGE_KEYS.decisionWheel, DEFAULTS),
    []
  );
  ...
  useEffect(() => {
    savePersistedJson(QUICK_TOOLS_STORAGE_KEYS.decisionWheel, { optionsText });
  }, [optionsText]);
  ...
  const drawWheel = useCallback((highlightIdx = -1) => {
    ...
    items.forEach((item, i) => {
      ...
      ctx.textAlign = 'right';
      ctx.fillStyle = isHighlight ? '#1e293b' : '#ffffff';
  ...
  const tick = useCallback(
    (now: number) => {
      ...
      if (confettiRef.current.length > 0) {
        confettiRef.current = tickConfetti(
          confettiRef.current,
          dt || 1 / 60,
          confettiCanvasRef.current?.height ?? 400
        );
        drawConfetti();
      }
    ...
    <div className="space-y-6">
      ...
      <section className="relative mx-auto max-w-md text-center">
        <div className="pointer-events-none absolute left-1/2 top-0 z-20 -translate-x-1/2 -translate-y-1">
          <div className="h-0 w-0 border-x-[14px] border-x-transparent border-t-[22px] border-t-violet-600 drop-shadow" />
        </div>
        ...
        <p className="mt-6 text-lg font-bold text-violet-800" aria-live="polite">
          {result}
        </p>
      </section>
    </div>
