...
type WorkerProgress = { current: number; total: number; label: string };
...
function getWorker(): Worker {
  if (!worker) {
    worker = new Worker(new URL('../workers/pdfProcessor.worker.ts', import.meta.url), {
      type: 'module',
    });
  }
  return worker;
...
/**
 * Stream a File/Blob into the worker in sequential chunks (Streams API).
 * The main thread never holds the full file buffer at once.
 */
...
/** Stream JPEG pages one at a time — only one page buffer lives in memory per side. */
...
/** @deprecated Prefer getPdfPageCountFromFile for large files. */
