...
/**
 * Read a File via the Streams API in fixed-size chunks without materializing
 * the full ArrayBuffer on the caller thread.
 */
export async function readFileInChunks(
  ...
  while (true) {
    ...
    const chunk = value.byteLength === value.buffer.byteLength
      ? value
      : value.slice();
...
/** Assemble chunks into a single pre-sized buffer (worker-side). */
