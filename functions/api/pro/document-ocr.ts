/**
 * POST /api/pro/document-ocr
 * Gemini 2.5 Flash-Lite layout-preserving OCR for Document Studio (Pro).
 *
 * Auth: Better Auth session + users.plan === 'pro' + sufficient AI Credits.
 * Secret: GEMINI_API_KEY (Cloudflare Pages environment variable).
 */
...
import {
  ...
  runGeminiDocumentOcr,
} from '../../_lib/geminiDocumentOcr.mjs';

const OPERATION_ID = 'document-ocr';
...
export async function onRequestPost(context) {
  ...
  // Explicit D1/Neon plan check — PRO status is users.plan === 'pro'
  ...
  try {
    ...
    console.error('[document-ocr] Failed to parse upload:', err);
  ...
  try {
    const result = await runGeminiDocumentOcr({
      apiKey,
    ...
    console.error('[document-ocr] Gemini OCR failed:', err);
    const code = err && typeof err === 'object' ? err.code : undefined;
    if (code === 'BAD_MIME' || code === 'BAD_PAYLOAD' || code === 'EMPTY') {
      return jsonResponse(
        { error: 'Bad Request', message: err.message || 'Document OCR failed.' },
        400,
      );
    }
    if (code === 'CONFIG') {
    ...
    return jsonResponse(
      {
        ...
        message: 'Vision AI could not read this document. Please try again.',
      },
      500,
    );
  }
